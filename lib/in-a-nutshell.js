...
function nutshell(text, summary_length, output_type) {
    /*
        Params:
        text (String) => The text to summarize
        summary_length (Integer) => The number of sentences the summary will contain
        output_type (String) => The output type of the summary (html or plaintext)
    */
    summary_length = summary_length > 0 ? summary_length : 4;
    output_type = output_type ? output_type : "plaintext";
    ...
    if(relevant_sentences.tier_1.length >= summary_length) {
        // Get relevant and unqiue sentences from word set based on determined keyphrase
        summary = relevant_sentences.tier_1.slice(0, summary_length).map(function(sent) {
            if(output_type == "html") {
                return "<p>" + sents[sent] + "</p>";
            }
            return sents[sent];
        });

    } else if(relevant_sentences.tier_2.length >= summary_length) {
        // TODO:
        // Create summary from tier 1 und tier 2 sentences
        summary = relevant_sentences.tier_2.slice(0, summary_length).map(function(sent) {
            if(output_type == "html") {
                return "<p>" + sents[sent] + "</p>";
            }
            return sents[sent];
    ...
    for(var i=0; i < sents.length; i++) {
        ...
        for(var i2=0; i2 < words.length; i2++) {
            ...
            if(stopwords.indexOf(word) === -1 && word.length > 2) {
                if(Array.isArray(word_set[word])) {
